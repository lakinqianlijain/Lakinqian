...
public class EmoticonManager {
    private static final String EMO_DIR = "emoji/";
    ...
    public static void loadEmoticon(int tabPosition){
        if (tabPosition == 0){
            load(JianApplication.get(), EMO_DIR + "emoji.xml");
        } if (tabPosition == 1){
            load(JianApplication.get(), EMO_DIR + "gif.xml");
        }
    ...
    private static final void load(Context context, String xmlPath) {
        ...
        //补充最后一页少的表情
        int tmp = mDefaultEntries.size() % EmoticonLayout.EMOTICON_PER_PAGE;
        if (tmp != 0) {
            int tmp2 = EmoticonLayout.EMOTICON_PER_PAGE - (mDefaultEntries.size() - (mDefaultEntries.size() / EmoticonLayout.EMOTICON_PER_PAGE) * EmoticonLayout.EMOTICON_PER_PAGE);
    ...
    private static class EntryLoader extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            if (localName.equals("Catalog")) {
                ...
                Entry entry = new Entry(tag, EMO_DIR + catalog + "/" + fileName);
